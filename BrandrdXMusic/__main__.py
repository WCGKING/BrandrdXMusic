...
import config
from ValenciaXMusic import LOGGER, app, userbot
from ValenciaXMusic.core.call import Hotty
from ValenciaXMusic.misc import sudo
from ValenciaXMusic.plugins import ALL_MODULES
from ValenciaXMusic.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("ValenciaXMusic.plugins" + all_module)
    LOGGER("ValenciaXMusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Hotty.start()
    try:
    ...
    await app.stop()
    await userbot.stop()
    LOGGER("ValenciaXMusic").info("Stopping Valencia Music Bot...")
